...
#
# Usage:
#   bash tools/quality/local-ci.sh           # launch the sweep detached, return immediately
#   bash tools/quality/local-ci.sh --status  # print per-gate status of the last/current run
#   bash tools/quality/local-ci.sh --run     # internal: execute the gates in foreground
#
...
show_status() {
    if [ ! -f "$STATUS_FILE" ]; then
        echo "no run found — launch with: bash tools/quality/local-ci.sh"
        exit 1
    fi
    cat "$STATUS_FILE"
    if [ -f "$HEAD_FILE" ]; then
...
case "${1:-}" in
    ...
    *)
        ...
        if [ -f "$PID_FILE" ] && kill -0 "$(cat "$PID_FILE")" 2>/dev/null; then
            echo "already running (pid $(cat "$PID_FILE")) — bash tools/quality/local-ci.sh --status"
            exit 1
        fi
        sha="$(current_head)" || {
        ...
        echo $! >"$PID_FILE"
        disown
        echo "launched (pid $(cat "$PID_FILE")) — check with: bash tools/quality/local-ci.sh --status"
        ;;
esac
