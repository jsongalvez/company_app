...
#
#   bash tools/performance/run-k6-contract-suites.sh
#
...
cleanup() {
    ...
    if ! bash tools/database/clean-test-db.sh 2>&1 | tail -3; then
...
for suite in remittance-race-test baseline authz-test concurrency-test full-suite; do
    ...
    set +e
    set -o pipefail
    k6 run "tools/performance/k6/$suite.js" --quiet 2>&1 | tee -a "$K6_LOG"
