...
Docs-only changes (`docs/**/*.md`, `.opencode/**/*.md`, `AGENTS.md`, `CONTEXT.md`,
`README*.md`, and `CHANGELOG.md`) skip local pre-push code, contract, Compose,
startup, and k6 gates. They still use the pull-request workflow by default.
Mixed or gate-sensitive pushes run the complete local pre-push checks, including
test-data cleanliness, OpenAPI, Compose desktop/Android compilation, and k6
against the disposable test database.

Emergency or AFK work does not bypass safety checks silently. Push to a branch,
open a draft or urgent pull request, record reason and owner in the pull request,
and leave a handoff when review or follow-up is unavailable. Do not merge with
failed checks or use force-push/history rewriting as an emergency shortcut.
