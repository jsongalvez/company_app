...
**What the wizard does — v2 (session 65, extended). Modes: `full` = wayfinder chain + Coolify (36-37 stages, ~155 min), `app-only` = just the app (21-22 stages, ~95 min); `tailnet` or `public` SSH. Run on THIS Arch box (the repo on the external mount):**

- Config (asked once, kept in `.wayfinder-vps.env`): MODE, SSH_MODE, TS_HOSTNAME, VPS_USER, REPO_URL, DEPLOY_BRANCH, APP_DOMAIN — the wizard is generic enough for the future app owner (their repo, their tailnet, their domain; nip.io derived when blank).
- Phase 0 — pre-flight: branch state + open issues; optional early push of `$DEPLOY_BRANCH` (full mode: ~276 ahead).
...
- Phase 2 — Tailscale: authkey (ask_secret, consumed once) → install + join tailnet on the VPS (hostname = TS_HOSTNAME, verified on re-runs) → `TS_IP` derived; **all later stages ride the tailnet** (ssh/scp helpers); **Disable Key Expiry stage** (180-day node-key expiry = a tailnet-only lockout).
- Phase 3 — bootstrap: base packages + docker enable (all modes), full mode adds JDK/tmux/npm/gh, `gh auth login --with-token` (PAT ask_secret, never written to disk), opencode2 pinned, opencode config + auth.json scp (**NOT** opencode.db), `opencode2 serve --service`, repo clone (graceful if the branch isn't on origin yet), `.env` + `.wayfinder-loop.env` scp, Postgres 18 compose up, setup-hooks, gate warm (nohup + poll + stall detection).
- Phase 4 — harden: ufw (default deny; tailnet0; 80/443; 41641/udp — public mode adds 51920), sshd `PermitRootLogin no` + `PasswordAuthentication no` (public mode: socket→service switch + dual Port 22+51920 + fail2ban jail on both + console-recovery password note), unattended-upgrades, Oracle security list (tailnet: drop 22; public: add 51920), **Oracle idle-reclaim heartbeat cron** (7-day idle stop; Coolify's memory footprint covers the memory leg).
- Phase 5 — Coolify: install, dashboard first run (admin email/password captured to `.wayfinder-vps.env`), **backend Dockerfile** (authored + committed this session — see below), Postgres resource (env values read live from local `.env` for copy-paste), app resource (Public Repository — no GitHub App needed, repo is public; branch `$DEPLOY_BRANCH`; Dockerfile build pack; env block printed from local `.env` with `DB_HOST=postgres` + `APP_HOST=0.0.0.0`; domain `$APP_DOMAIN` or derived `api.<VPS_IP>.nip.io` + Let's Encrypt; real-domain DNS note; remote-origin mismatch warn), `/health` HTTPS verification.
- Phase 6 — the switch (full mode only; CRITICAL — never two daemons): boundary check (latest handoff detected from `docs/agents/`), **kill local daemon FIRST, then push** (the push carries anything a session committed up to the boundary; handoff re-derived after the kill + verified by sha256 on the VPS before bootstrap), VPS daemon via `tmux new-session -d … --bootstrap <handoff>`, verify `spawned ses_` in `.wayfinder-loop.log` + ntfy confirm, post checks.
...
**v2 review (the same loop, 6 passes; commits `b60fd51` → `9e1a3a2`):** pass 1 → 2 HARD (REPO-before-ENV_FILE startup crash — 3-phase sighting, empirically confirmed; stage-count truth-class) + fail2ban jail-port gap (default jail banned only 22 — the public-mode claim was false); pass 2 → 3 HARD (first-ever-push gap at the switch — P3+P4; Ubuntu 24.04 socket-activation silently ignored sshd_config Port — 51920 never bound; app-only docker enablement); pass 3 → 3 HARD (explicit restart needed after socket disable; unguarded fatal in the ssh-reconfig arm; non-fast-forward push dead-end); pass 4 → 0 HARD (P4's "app=31" count claim rejected with evidence: 37 lines − 15 full-only − 1 public-only = 21); pass 5 → 1 HARD (Cloud Shell fallback was dead at the ufw layer — the serial console is the live path); pass 6 → 0 HARD — **EXIT**. P5: 4 ARCH — 1 fixed in-ticket (`env_val` helper — the 12 duplicated .env-read pipes) + 3 graduates folded into the template fog line (push-gate unifier, sshd-arm parametrization, config-block `ask_env` shape; `set_secret`/`set_var` dead-by-design rides it). P5 loop-back verification: clean. Accepted SOFTs carried from v1 (a)(b)(c)(g)(h) + (i) fetch-failure-silent-skip — all dispositioned (downstream sha gate catches (i); (b) rides the template graduate).
...
Standing fog lines (for sessions AFTER the migration lands): the P5 ARCH graduates (mirror-only KeepLastByKey split — marker: third consumer or ReliefInvite touching `lastByKey` without the guard; handler state-less launch — kills the #163 snag; update-one map-if sub-shape; stale-response guards — DOCUMENTATION ONLY; unifier-family-naming-coherence — `ActionTracker.begin` vs `tryBegin`, rides the mirror-only split), **wizard-template helpers (NEW — the #168 P5 graduate, now absorbing THREE more v2 graduates: the library should grow `_wait_until`, a documented guarded-ssh/`vps`-retry idiom, an `ensure_pushed` push-gate helper, an `ask_env` config-block helper, and fold `set_secret`/`set_var` dead-by-design into the template)**, DayStatus shared/domain move, #92 Q4/Q5 403-refresh machinery, #158 ARCH pair + #160 graduates (ilike unifier; screen-scoped-VM-seam), request-flow grant/deny UI gap, k6 for branch-scoped writes (needs #106 seeding), then #110 / root-AGENTS.md pre-push-iOS line / #139 / #106.
