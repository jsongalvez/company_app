...
| `tools/wayfinder/wayfinder-chief.sh` | Map chief scheduler (map #697 #735 #737 #738 #739 #740 #741): reconcile → collect (`done`/`blocked`/`failed` → `ready-for-review` for #740) → authoritative frontier query (open blockers, claims, `needs-info`/`ready-for-human`, priority order) → fill free capacity → idle work, looping (`--once` for a single pass, `--dry-run` side-effect free, queue depth logged every pass). Ticket prompts carry the #737 worker contract (role/ticket/map/workspace-base identity, assign-first claim with race fail-safe, one-ticket boundary, structured STATUS report, `blocked`/HELP REQUESTS protocol); `--spawn-helper` performs one chief-mediated helper spawn (parent association, bounded scope, capacity-counted); `--spawn-scout` performs one chief-mediated bug-scout spawn for a bounded audit slice (prompt + progress via `wayfinder-scout.sh`, map-anchored `bug-scout` worker row); `--spawn-maintenance` performs one chief-mediated maintenance spawn for a red SHA (prompt via `wayfinder-maintenance.sh`, repair issue as its ticket, isolated workspace, `WAYFINDER_MAX_MAINTENANCE_WORKERS` narrow bound, no duplicate dispatch per repair, integration-gated log); `--recover` reconciles worker + provider state and prints orphan release guidance before any dispatch (no fill). Workers claim assign-first; the chief never assigns and writes canonical state only through the review queue's serialized `--integrate` lane (single-shot `--queue`/`--review`/`--integrate` lanes delegate to `wayfinder-review.sh`). Self-test: `wayfinder-chief-test.sh`. |
