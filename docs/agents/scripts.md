...
| `tools/wayfinder/wayfinder-chief.sh` | Map chief scheduler (map #697 #735 #737): reconcile → collect (`done`/`blocked` → `ready-for-review` for #740) → authoritative frontier query (open blockers, claims, `needs-info`/`ready-for-human`, priority order) → fill free capacity → idle work, looping (`--once` for a single pass, `--dry-run` side-effect free). Ticket prompts carry the #737 worker contract (role/ticket/map/workspace-base identity, assign-first claim with race fail-safe, one-ticket boundary, structured STATUS report, `blocked`/HELP REQUESTS protocol); `--spawn-helper` performs one chief-mediated helper spawn (parent association, bounded scope, capacity-counted). Workers claim assign-first; the chief never assigns and never runs Git. Self-test: `wayfinder-chief-test.sh`. |
