...
| truth-class | a claim or doc line contradicted by shipped code — resolution comments, ADR/AGENTS.md/KDoc | #145 (2 HARD); #146 (1 HARD-class); #148 (ticket+KDoc claimed "serialName + optional `?arg={arg}` — verified"; `RouteBuilder` emits non-optional args as `/` path segments); later bumps — #170 (the "sole onError-less stateless site" premise, 2×); #172 ("only HTTP-status VM pins" overclaim); #173 (fog's fictional "6+"/"flagged-shared" stale-gate count — corrected pre-claim; the driver's own reworded test comment claiming coroutine-start eval was pinned — caught by the lenses pre-ship) |
...
| loud-stop swallow | an `abort()` (or any warn+exit loud-stop) invoked inside a `$( )` substitution — the message is captured into the substitution output and never reaches the terminal; `exit` kills only the subshell; the failing assignment then trips `set -e` and the script dies silently. Fix shape: never wrap abort-able helpers in `$()` — call as a plain statement that sets a global (and warn future call sites in the helper's comment). | wayfinder-169 P5 loop-back (1 — the wizard's `latest_handoff` helper, empirically confirmed silent-death on both failure branches) |
...
If the ticket defines executable acceptance criteria, the driving agent runs them once per
pass — the verdict is the mechanical half of spec conformance. There is no per-ticket gate
ledger: acceptance criteria live in the GitHub issue; focused negative controls (proving a
regression test is meaningful) run when they answer the ticket's question and their result is
recorded in the resolution comment. Your judgment budget goes to reading the ticket and the
composed code/tests, not to re-reading acceptance the machine already decided.
