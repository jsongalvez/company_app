...
- **Chain-end fallback**: when zero open `wayfinder:map` exist, chain-end is not a stop — claim exactly one open takeable AFK task instead of exiting (that pickup IS the session's one claim). Takeable means open, unassigned, no open blocker, and free of `ready-for-human` / `needs-info` / `needs-triage`; never pick a child of a CLOSED map or a retired marker (e.g. `<!-- wayfinder-local-ci-repair -->`). Prefer `ready-for-agent`; a well-specified untagged follow-up with clear acceptance also qualifies. Lowest number breaks ties.
