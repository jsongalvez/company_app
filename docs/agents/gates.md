...
3. **Before claiming done**: re-run the checker — every box checked with evidence, then run
   selected risk-based review profile (`AGENTS.md`, `docs/agents/code-review-loop.md`). There is
   no mandatory full compile/test gate at integration; targeted checks + asynchronous CI carry
   verification (map #329).
