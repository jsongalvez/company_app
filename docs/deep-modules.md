...
**Public seam:** `currentOperationalDate` · `resolveOrCreate` / `getToday` / `findToday` / `findByBranchAndDate` (resolve-or-create vs find-only discipline matters) · `requireBranchDayExists` / `requireBranchDayForBranch` · `checkBranchDayEditable` / `checkBranchDayReadable` · `evaluateStatus` · `markDaysRemittedInTransaction` / `releaseDaysFromRemittanceInTransaction` · `expirationUtc` · `manilaZone`.
