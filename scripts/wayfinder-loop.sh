...
supervise_session() {
  ...
  while :; do
    ...
    if [ -n "$stop_message" ] && [ "$stop_message" != "$last_stop_message" ]; then
      if api post "/api/session/$session_id/prompt" --data "$(jq -nc '{text: "You stopped without writing the required handoff. Continue from Map #180 workflow authority. If frontier exists, claim and resolve one child. If frontier is empty, run the required focused/full audit, complete verifier packets for every retained candidate, and graduate one candidate only after all packets are complete, or record clean-audit evidence. Then write the handoff last."}')" >/dev/null 2>&1; then
