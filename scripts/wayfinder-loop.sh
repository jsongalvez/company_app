...
CI_SCRIPT="$REPO/scripts/wayfinder-ci.sh"
...
checkpoint_handoff() {
  local doc="$1"
  if [ -n "${WAYFINDER_TICKET:-}" ]; then
    "$CI_SCRIPT" validate-branch-dirty "$WAYFINDER_TICKET" || return 1
  fi
...
spawn_session() {
  ...
  if [ -z "${WAYFINDER_ALLOW_DIRTY:-}" ]; then
    [ -n "${WAYFINDER_TICKET:-}" ] || die "WAYFINDER_TICKET is required for AFK branch ownership"
    "$CI_SCRIPT" prepare "$WAYFINDER_TICKET"
...
[ -x "$CI_SCRIPT" ] || die "Wayfinder CI contract missing or not executable: $CI_SCRIPT"
