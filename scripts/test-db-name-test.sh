...
run_case() {
    ...
    if [ -n "$test_db" ]; then
        actual="$(env -i PATH="$PATH" POSTGRES_DB="$postgres_db" TEST_DB_NAME="$test_db" \
            bash -c "source '$ROOT_DIR/scripts/lib/common.sh'; test_db_name")"
    else
...
selected="$(env -i PATH="$PATH" POSTGRES_DB=company_app bash -c \
    "source '$ROOT_DIR/scripts/lib/common.sh'; K6_DB_NAME=\"\${TEST_DB_NAME:-company_app_test}\"; export TEST_DB_NAME=\"\$K6_DB_NAME\"; test_db_name")"
