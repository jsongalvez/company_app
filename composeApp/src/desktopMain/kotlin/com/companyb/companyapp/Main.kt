...
import com.companyb.companyapp.proto.timeline.ProtoTimelineApp
...
fun main() {
    ...
    application {
        val protoTimeline = System.getenv("COMPANYAPP_PROTO_TIMELINE") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoTimeline) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoTimeline) "CompanyApp — Timeline Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoTimeline) {
                ProtoTimelineApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
