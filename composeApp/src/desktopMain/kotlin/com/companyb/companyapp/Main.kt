...
import com.companyb.companyapp.proto.roundscart.ProtoRoundsCartApp
...
fun main() {
    ...
    application {
        val protoRoundsCart = System.getenv("COMPANYAPP_PROTO_ROUNDS_CART") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoRoundsCart) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoRoundsCart) "CompanyApp — rounds-cart prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoRoundsCart) {
                ProtoRoundsCartApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
