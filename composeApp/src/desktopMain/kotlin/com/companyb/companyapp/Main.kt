...
import com.companyb.companyapp.proto.textonly.ProtoTextOnlyApp
...
fun main() {
    ...
    application {
        val protoTextOnly = System.getenv("COMPANYAPP_PROTO_TEXT_ONLY") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoTextOnly) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoTextOnly) "CompanyApp — Text-Only Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoTextOnly) {
                ProtoTextOnlyApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
