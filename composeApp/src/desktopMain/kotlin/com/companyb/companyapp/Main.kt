...
import com.companyb.companyapp.proto.printledger.PrintLedgerProtoApp
...
fun main() {
    ...
    application {
        val protoPrintLedger = System.getenv("COMPANYAPP_PROTO_PRINT_LEDGER") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoPrintLedger) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoPrintLedger) "CompanyApp — Print Ledger Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoPrintLedger) {
                PrintLedgerProtoApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
