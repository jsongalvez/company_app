...
import com.companyb.companyapp.proto.blueprintdraft.BlueprintDraftProtoApp
...
fun main() {
    ...
    val blueprintDraft = System.getenv("COMPANYAPP_PROTO_BLUEPRINT_DRAFT") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (blueprintDraft) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (blueprintDraft) "CompanyApp - Blueprint Draft prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (blueprintDraft) {
                BlueprintDraftProtoApp()
            } else {
                App()
            }
        }
