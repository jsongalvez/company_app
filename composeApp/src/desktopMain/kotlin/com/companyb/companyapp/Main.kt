...
import com.companyb.companyapp.proto.reliefnetwork.ProtoReliefNetworkApp
...
fun main() {
    ...
    val protoReliefNetwork = System.getenv("COMPANYAPP_PROTO_RELIEF_NETWORK") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (protoReliefNetwork) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoReliefNetwork) "CompanyApp — Relief-Network Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoReliefNetwork) {
                ProtoReliefNetworkApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
