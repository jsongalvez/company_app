...
import com.companyb.companyapp.proto.inboxzero.ProtoInboxZeroApp
...
fun main() {
    ...
    application {
        val protoInboxZero = System.getenv("COMPANYAPP_PROTO_INBOX_ZERO") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoInboxZero) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoInboxZero) "CompanyApp — Inbox Zero Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoInboxZero) {
                ProtoInboxZeroApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
