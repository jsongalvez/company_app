...
import com.companyb.companyapp.proto.swissgrid.ProtoSwissGridApp
...
fun main() {
    ...
    application {
        val swissGrid = System.getenv("COMPANYAPP_PROTO_SWISS_GRID") == "true"
        val windowState =
            rememberWindowState(
                size = if (swissGrid) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (swissGrid) "CompanyApp — swiss-grid prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (swissGrid) {
                ProtoSwissGridApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
