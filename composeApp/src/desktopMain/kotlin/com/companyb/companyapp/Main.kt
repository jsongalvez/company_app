...
import com.companyb.companyapp.proto.darkops.DarkOpsApp
...
fun main() {
    ...
    val darkOps = System.getenv("COMPANYAPP_PROTO_DARK_OPS") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (darkOps) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (darkOps) "CompanyApp // dark-ops" else "CompanyApp",
            state = windowState,
        ) {
            if (darkOps) {
                DarkOpsApp()
