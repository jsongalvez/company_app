...
import com.companyb.companyapp.proto.terminalgreen.ProtoTerminalGreenApp
...
fun main() {
    ...
    application {
        val protoTerminalGreen = System.getenv("COMPANYAPP_PROTO_TERMINAL_GREEN") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoTerminalGreen) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoTerminalGreen) "CompanyApp — Terminal-Green Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoTerminalGreen) {
                ProtoTerminalGreenApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
