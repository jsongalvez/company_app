...
import com.companyb.companyapp.proto.zenfocus.ZenFocusApp
...
fun main() {
    ...
    application {
        val zenFocus = System.getenv("COMPANYAPP_PROTO_ZEN_FOCUS") == "true"
        val windowState =
            rememberWindowState(
                size = if (zenFocus) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (zenFocus) "CompanyApp — zen-focus prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (zenFocus) {
                ZenFocusApp()
