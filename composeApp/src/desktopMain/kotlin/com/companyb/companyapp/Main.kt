...
import com.companyb.companyapp.proto.lunchrush.LunchRushApp
...
fun main() {
    ...
    val lunchRush = System.getenv("COMPANYAPP_PROTO_LUNCH_RUSH") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (lunchRush) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (lunchRush) "CompanyApp // lunch-rush" else "CompanyApp",
            state = windowState,
        ) {
            if (lunchRush) {
                LunchRushApp()
