...
import com.companyb.companyapp.proto.neurosoft.NeuroSoftProtoApp
...
fun main() {
    ...
    val neuroSoft = System.getenv("COMPANYAPP_PROTO_NEURO_SOFT") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (neuroSoft) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (neuroSoft) "CompanyApp - Neuro Soft prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (neuroSoft) {
                NeuroSoftProtoApp()
