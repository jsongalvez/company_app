...
import com.companyb.companyapp.proto.solarizedcalm.SolarizedCalmApp
...
fun main() {
    ...
    application {
        val solarizedCalm = System.getenv("COMPANYAPP_PROTO_SOLARIZED_CALM") == "true"
        val windowState =
            rememberWindowState(
                size = if (solarizedCalm) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (solarizedCalm) "CompanyApp — solarized-calm prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (solarizedCalm) {
                SolarizedCalmApp()
