...
import com.companyb.companyapp.proto.dualpersona.DualPersonaProtoApp
...
fun main() {
    ...
    application {
        val protoDualPersona = System.getenv("COMPANYAPP_PROTO_DUAL_PERSONA") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoDualPersona) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoDualPersona) "CompanyApp — Dual Persona Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoDualPersona) {
                DualPersonaProtoApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
