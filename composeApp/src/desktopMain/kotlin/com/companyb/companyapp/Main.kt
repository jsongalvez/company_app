...
import com.companyb.companyapp.proto.commanddeck.CommandDeckApp
...
fun main() {
    ...
    application {
        val commandDeck = System.getenv("COMPANYAPP_PROTO_COMMAND_DECK") == "true"
        val windowState =
            rememberWindowState(
                size = if (commandDeck) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (commandDeck) "CompanyApp — command-deck prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (commandDeck) {
                CommandDeckApp()
