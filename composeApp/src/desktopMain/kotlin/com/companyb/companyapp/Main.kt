...
import com.companyb.companyapp.proto.queuedesk.ProtoQueueDeskApp
...
fun main() {
    ...
    application {
        val protoQueueDesk = System.getenv("COMPANYAPP_PROTO_QUEUE_DESK") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoQueueDesk) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoQueueDesk) "CompanyApp — queue-desk prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoQueueDesk) {
                ProtoQueueDeskApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
