...
import com.companyb.companyapp.proto.auditordense.AuditorDenseApp
...
fun main() {
    ...
    application {
        val auditorDense = System.getenv("COMPANYAPP_PROTO_AUDITOR_DENSE") == "true"
        val windowState =
            rememberWindowState(
                size = if (auditorDense) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (auditorDense) "CompanyApp — auditor-dense prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (auditorDense) {
                AuditorDenseApp()
