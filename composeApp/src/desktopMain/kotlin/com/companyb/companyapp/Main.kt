...
import com.companyb.companyapp.proto.cashdrawer.ProtoCashDrawerApp
...
fun main() {
    ...
    application {
        val protoCashDrawer = System.getenv("COMPANYAPP_PROTO_CASH_DRAWER") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoCashDrawer) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoCashDrawer) "CompanyApp — Cash-Drawer Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoCashDrawer) {
                ProtoCashDrawerApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
