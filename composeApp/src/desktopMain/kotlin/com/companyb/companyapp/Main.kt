...
import com.companyb.companyapp.proto.opsfeed.ProtoOpsFeedApp
...
fun main() {
    ...
    application {
        val protoOpsFeed = System.getenv("COMPANYAPP_PROTO_OPS_FEED") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoOpsFeed) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoOpsFeed) "CompanyApp — Ops Feed Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoOpsFeed) {
                ProtoOpsFeedApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
