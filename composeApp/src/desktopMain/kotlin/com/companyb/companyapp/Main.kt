...
import com.companyb.companyapp.proto.kiosktouch.KioskTouchApp
...
fun main() {
    ...
    application {
        val kioskTouch = System.getenv("COMPANYAPP_PROTO_KIOSK_TOUCH") == "true"
        val windowState =
            rememberWindowState(
                size = if (kioskTouch) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (kioskTouch) "CompanyApp — kiosk-touch prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (kioskTouch) {
                KioskTouchApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
