...
import com.companyb.companyapp.proto.bottomdock.BottomDockApp
...
fun main() {
    ...
    application {
        val bottomDock = System.getenv("COMPANYAPP_PROTO_BOTTOM_DOCK") == "true"
        val windowState =
            rememberWindowState(
                size = if (bottomDock) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (bottomDock) "CompanyApp — bottom-dock prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (bottomDock) {
                BottomDockApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
