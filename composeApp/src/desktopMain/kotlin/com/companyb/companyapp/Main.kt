...
import com.companyb.companyapp.proto.wireframelive.WireframeLiveProtoApp
...
fun main() {
    ...
    val wireframeLive = System.getenv("COMPANYAPP_PROTO_WIREFRAME_LIVE") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (wireframeLive) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (wireframeLive) "CompanyApp - Wireframe-Live prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (wireframeLive) {
                WireframeLiveProtoApp()
            } else {
                App()
            }
        }
