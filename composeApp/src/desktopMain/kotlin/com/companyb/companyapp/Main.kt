...
import com.companyb.companyapp.proto.guided_onboarding.ProtoGuidedOnboardingApp
...
fun main() {
    ...
    application {
        val protoGuidedOnboarding = System.getenv("COMPANYAPP_PROTO_GUIDED_ONBOARDING") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoGuidedOnboarding) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoGuidedOnboarding) "CompanyApp — Guided Onboarding Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoGuidedOnboarding) {
                ProtoGuidedOnboardingApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
