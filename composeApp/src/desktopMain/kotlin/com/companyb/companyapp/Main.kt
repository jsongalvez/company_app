...
import com.companyb.companyapp.proto.nordfrost.NordFrostProtoApp
...
fun main() {
    ...
    val nordFrost = System.getenv("COMPANYAPP_PROTO_NORD_FROST") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (nordFrost) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (nordFrost) "CompanyApp - Nord-Frost prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (nordFrost) {
                NordFrostProtoApp()
