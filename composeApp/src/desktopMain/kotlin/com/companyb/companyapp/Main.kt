...
import com.companyb.companyapp.proto.sheetgrid.SheetGridApp
...
fun main() {
    ...
    val sheetGrid = System.getenv("COMPANYAPP_PROTO_SHEET_GRID") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (sheetGrid) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (sheetGrid) "CompanyApp — sheet-grid prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (sheetGrid) {
                SheetGridApp()
