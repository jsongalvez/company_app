...
import com.companyb.companyapp.proto.forestcalm.ForestCalmApp
...
fun main() {
    ...
    application {
        val forestCalm = System.getenv("COMPANYAPP_PROTO_FOREST_CALM") == "true"
        val windowState =
            rememberWindowState(
                size = if (forestCalm) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (forestCalm) "CompanyApp — forest-calm prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (forestCalm) {
                ForestCalmApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
