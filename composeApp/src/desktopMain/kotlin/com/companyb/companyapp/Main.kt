...
import com.companyb.companyapp.proto.ownerdesk.ProtoOwnerDeskApp
...
fun main() {
    ...
    application {
        val protoOwnerDesk = System.getenv("COMPANYAPP_PROTO_OWNER_DESK") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoOwnerDesk) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoOwnerDesk) "CompanyApp — Owner's Desk Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoOwnerDesk) {
                ProtoOwnerDeskApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
