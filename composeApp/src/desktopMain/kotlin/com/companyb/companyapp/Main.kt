...
import com.companyb.companyapp.proto.shifthandover.ProtoShiftHandoverApp
...
fun main() {
    ...
    application {
        val protoShiftHandover = System.getenv("COMPANYAPP_PROTO_SHIFT_HANDOVER") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoShiftHandover) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoShiftHandover) "CompanyApp — Shift Handover Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoShiftHandover) {
                ProtoShiftHandoverApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
