...
import com.companyb.companyapp.proto.coachmarks.CoachMarksApp
...
fun main() {
    ...
    application {
        val coachMarks = System.getenv("COMPANYAPP_PROTO_COACH_MARKS") == "true"
        val windowState =
            rememberWindowState(
                size = if (coachMarks) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (coachMarks) "CompanyApp — coach-marks prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (coachMarks) {
                CoachMarksApp()
