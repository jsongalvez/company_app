...
import com.companyb.companyapp.proto.delegationboard.DelegationBoardProtoApp
...
fun main() {
    ...
    val delegationBoard = System.getenv("COMPANYAPP_PROTO_DELEGATION_BOARD") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (delegationBoard) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (delegationBoard) "CompanyApp - Delegation Board prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (delegationBoard) {
                DelegationBoardProtoApp()
            } else {
                App()
            }
        }
