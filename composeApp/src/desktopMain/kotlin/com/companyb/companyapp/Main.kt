...
import com.companyb.companyapp.proto.guestpass.GuestPassApp
...
fun main() {
    ...
    application {
        val guestPass = System.getenv("COMPANYAPP_PROTO_GUEST_PASS") == "true"
        val windowState =
            rememberWindowState(
                size = if (guestPass) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (guestPass) "CompanyApp — guest-pass prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (guestPass) {
                GuestPassApp()
