...
import com.companyb.companyapp.proto.desertsand.DesertSandApp
...
fun main() {
    ...
    application {
        val desertSand = System.getenv("COMPANYAPP_PROTO_DESERT_SAND") == "true"
        val windowState =
            rememberWindowState(
                size = if (desertSand) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (desertSand) "CompanyApp — desert-sand prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (desertSand) {
                DesertSandApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
