...
import com.companyb.companyapp.proto.warmcare.WarmCareProtoApp
...
fun main() {
    ...
    val warmCare = System.getenv("COMPANYAPP_PROTO_WARM_CARE") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (warmCare) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (warmCare) "CompanyApp - Warm Care prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (warmCare) {
                WarmCareProtoApp()
