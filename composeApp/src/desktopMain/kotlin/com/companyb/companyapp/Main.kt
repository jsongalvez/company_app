...
import com.companyb.companyapp.proto.tradingdesk.ProtoTradingDeskApp
...
fun main() {
    ...
    application {
        val protoTradingDesk = System.getenv("COMPANYAPP_PROTO_TRADING_DESK") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoTradingDesk) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoTradingDesk) "CompanyApp — trading-desk prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoTradingDesk) {
                ProtoTradingDeskApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
