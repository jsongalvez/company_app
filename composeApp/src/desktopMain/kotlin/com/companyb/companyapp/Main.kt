...
import com.companyb.companyapp.proto.branchcompare.BranchCompareProtoApp
...
fun main() {
    ...
    val branchCompare = System.getenv("COMPANYAPP_PROTO_BRANCH_COMPARE") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (branchCompare) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (branchCompare) "CompanyApp - Branch Compare prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (branchCompare) {
                BranchCompareProtoApp()
            } else {
                App()
            }
        }
