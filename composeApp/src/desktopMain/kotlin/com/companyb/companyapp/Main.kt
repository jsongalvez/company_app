...
import com.companyb.companyapp.proto.financecockpit.FinanceCockpitApp
...
fun main() {
    ...
    application {
        val financeCockpit = System.getenv("COMPANYAPP_PROTO_FINANCE_COCKPIT") == "true"
        val windowState =
            rememberWindowState(
                size = if (financeCockpit) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (financeCockpit) "CompanyApp — finance-cockpit prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (financeCockpit) {
                FinanceCockpitApp()
