...
import com.companyb.companyapp.proto.listeverything.ProtoListEverythingApp
...
fun main() {
    ...
    application {
        val protoListEverything = System.getenv("COMPANYAPP_PROTO_LIST_EVERYTHING") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoListEverything) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoListEverything) "CompanyApp — List Everything Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoListEverything) {
                ProtoListEverythingApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
