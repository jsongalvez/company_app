...
import com.companyb.companyapp.proto.captainchair.ProtoCaptainChairApp
...
fun main() {
    ...
    application {
        val protoCaptainChair = System.getenv("COMPANYAPP_PROTO_CAPTAIN_CHAIR") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoCaptainChair) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoCaptainChair) "CompanyApp — Captain Chair Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoCaptainChair) {
                ProtoCaptainChairApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
