...
import com.companyb.companyapp.proto.duotonesea.SeaApp
...
fun main() {
    ...
    val duotoneSea = System.getenv("COMPANYAPP_PROTO_DUOTONE_SEA") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (duotoneSea) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (duotoneSea) "CompanyApp — duotone-sea prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (duotoneSea) {
                SeaApp()
