...
import com.companyb.companyapp.proto.compactlaptop.CompactLaptopProtoApp
...
fun main() {
    ...
    application {
        val protoCompactLaptop = System.getenv("COMPANYAPP_PROTO_COMPACT_LAPTOP") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoCompactLaptop) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoCompactLaptop) "CompanyApp — Compact Laptop Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoCompactLaptop) {
                CompactLaptopProtoApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
