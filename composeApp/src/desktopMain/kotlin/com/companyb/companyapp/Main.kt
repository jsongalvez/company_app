...
import com.companyb.companyapp.proto.monoink.MonoInkApp
...
fun main() {
    ...
    application {
        val monoInk = System.getenv("COMPANYAPP_PROTO_MONO_INK") == "true"
        val windowState =
            rememberWindowState(
                size = if (monoInk) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (monoInk) "CompanyApp — mono-ink prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (monoInk) {
                MonoInkApp()
