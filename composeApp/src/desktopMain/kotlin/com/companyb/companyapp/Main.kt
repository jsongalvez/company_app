...
import com.companyb.companyapp.proto.contextrail.ContextRailApp
...
fun main() {
    ...
    val contextRail = System.getenv("COMPANYAPP_PROTO_CONTEXT_RAIL") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (contextRail) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (contextRail) "CompanyApp — context-rail prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (contextRail) {
                ContextRailApp()
