...
import com.companyb.companyapp.proto.nightshift.ProtoNightShiftApp
...
fun main() {
    ...
    application {
        val protoNightShift = System.getenv("COMPANYAPP_PROTO_NIGHT_SHIFT") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoNightShift) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoNightShift) "CompanyApp — Night-Shift Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoNightShift) {
                ProtoNightShiftApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
