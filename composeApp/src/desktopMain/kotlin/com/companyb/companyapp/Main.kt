...
import com.companyb.companyapp.proto.mindmapnav.MindMapApp
...
fun main() {
    ...
    application {
        val mindmapNav = System.getenv("COMPANYAPP_PROTO_MINDMAP_NAV") == "true"
        val windowState =
            rememberWindowState(
                size = if (mindmapNav) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (mindmapNav) "CompanyApp — mindmap-nav prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (mindmapNav) {
                MindMapApp()
