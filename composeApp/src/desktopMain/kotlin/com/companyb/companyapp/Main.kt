...
import com.companyb.companyapp.proto.floatingpanels.FloatApp
...
fun main() {
    ...
    application {
        val floatingPanels = System.getenv("COMPANYAPP_PROTO_FLOATING_PANELS") == "true"
        val windowState =
            rememberWindowState(
                size = if (floatingPanels) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (floatingPanels) "CompanyApp — floating-panels prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (floatingPanels) {
                FloatApp()
