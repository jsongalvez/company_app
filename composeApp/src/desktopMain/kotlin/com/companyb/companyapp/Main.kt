...
import com.companyb.companyapp.proto.papercraft.PaperCraftProtoApp
...
fun main() {
    ...
    application {
        val protoPaperCraft = System.getenv("COMPANYAPP_PROTO_PAPER_CRAFT") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoPaperCraft) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoPaperCraft) "CompanyApp — Paper Craft Desk Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoPaperCraft) {
                PaperCraftProtoApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
