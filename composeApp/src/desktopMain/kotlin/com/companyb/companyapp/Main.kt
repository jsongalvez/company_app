...
import com.companyb.companyapp.proto.mochapop.MochaPopProtoApp
...
fun main() {
    ...
    val mochaPop = System.getenv("COMPANYAPP_PROTO_MOCHA_POP") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (mochaPop) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (mochaPop) "CompanyApp - Mocha Pop prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (mochaPop) {
                MochaPopProtoApp()
