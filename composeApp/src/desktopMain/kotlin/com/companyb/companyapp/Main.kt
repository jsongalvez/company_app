...
import com.companyb.companyapp.proto.glasspanels.GlassPanelsProtoApp
...
fun main() {
    ...
    val glassPanels = System.getenv("COMPANYAPP_PROTO_GLASS_PANELS") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (glassPanels) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (glassPanels) "CompanyApp - Glass Panels prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (glassPanels) {
                GlassPanelsProtoApp()
