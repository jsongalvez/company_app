...
import com.companyb.companyapp.proto.graveyardcalm.GraveyardCalmApp
...
fun main() {
    ...
    val graveyardCalm = System.getenv("COMPANYAPP_PROTO_GRAVEYARD_CALM") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (graveyardCalm) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (graveyardCalm) "CompanyApp // graveyard-calm" else "CompanyApp",
            state = windowState,
        ) {
            if (graveyardCalm) {
                GraveyardCalmApp()
