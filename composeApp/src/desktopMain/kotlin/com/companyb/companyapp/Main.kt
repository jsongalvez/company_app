...
import com.companyb.companyapp.proto.splitmaster.SplitMasterProtoApp
...
fun main() {
    ...
    application {
        val splitMaster = System.getenv("COMPANYAPP_PROTO_SPLIT_MASTER") == "true"
        val windowState =
            rememberWindowState(
                size = if (splitMaster) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (splitMaster) "CompanyApp — split-master prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (splitMaster) {
                SplitMasterProtoApp(onExit = ::exitApplication)
            } else {
                App()
            }
        }
