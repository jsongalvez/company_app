...
import com.companyb.companyapp.proto.brutalistraw.BrutalistRawProtoApp
...
fun main() {
    ...
    val brutalistRaw = System.getenv("COMPANYAPP_PROTO_BRUTALIST_RAW") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (brutalistRaw) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (brutalistRaw) "CompanyApp - Brutalist Raw prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (brutalistRaw) {
                BrutalistRawProtoApp()
            } else {
                App()
            }
        }
