...
import com.companyb.companyapp.proto.ribbontabs.RibbonApp
...
fun main() {
    ...
    application {
        val ribbonTabs = System.getenv("COMPANYAPP_PROTO_RIBBON_TABS") == "true"
        val windowState =
            rememberWindowState(
                size = if (ribbonTabs) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (ribbonTabs) "CompanyApp — ribbon-tabs prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (ribbonTabs) {
                RibbonApp()
