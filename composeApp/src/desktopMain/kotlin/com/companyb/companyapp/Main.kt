...
import com.companyb.companyapp.proto.noirdetective.NoirApp
...
fun main() {
    ...
    val noirDetective = System.getenv("COMPANYAPP_PROTO_NOIR_DETECTIVE") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (noirDetective) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (noirDetective) "CompanyApp // noir-detective" else "CompanyApp",
            state = windowState,
        ) {
            if (noirDetective) {
                NoirApp()
