...
import com.companyb.companyapp.proto.searchonly.ProtoSearchOnlyApp
...
fun main() {
    ...
    application {
        val protoSearchOnly = System.getenv("COMPANYAPP_PROTO_SEARCH_ONLY") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoSearchOnly) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoSearchOnly) "CompanyApp — Search Only Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoSearchOnly) {
                ProtoSearchOnlyApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
