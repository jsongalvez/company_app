...
import com.companyb.companyapp.proto.metricsmosaic.ProtoMetricsMosaicApp
...
fun main() {
    ...
    application {
        val protoMetricsMosaic = System.getenv("COMPANYAPP_PROTO_METRICS_MOSAIC") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoMetricsMosaic) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoMetricsMosaic) "CompanyApp — Metrics-Mosaic Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoMetricsMosaic) {
                ProtoMetricsMosaicApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
