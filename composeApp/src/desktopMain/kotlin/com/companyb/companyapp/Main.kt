...
import com.companyb.companyapp.proto.blockbuilder.BlockBuilderProtoApp
...
fun main() {
    ...
    val blockBuilder = System.getenv("COMPANYAPP_PROTO_BLOCK_BUILDER") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (blockBuilder) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (blockBuilder) "CompanyApp - Block Builder prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (blockBuilder) {
                BlockBuilderProtoApp()
            } else {
                App()
            }
        }
