...
import com.companyb.companyapp.proto.executivebrief.ProtoExecutiveBriefApp
...
fun main() {
    ...
    application {
        val protoExecutiveBrief = System.getenv("COMPANYAPP_PROTO_EXECUTIVE_BRIEF") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoExecutiveBrief) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoExecutiveBrief) "CompanyApp — Morning Brief Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoExecutiveBrief) {
                ProtoExecutiveBriefApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
