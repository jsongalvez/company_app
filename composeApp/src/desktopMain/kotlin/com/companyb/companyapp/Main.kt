...
import com.companyb.companyapp.proto.comicbold.ComicBoldProtoApp
...
fun main() {
    ...
    val comicBold = System.getenv("COMPANYAPP_PROTO_COMIC_BOLD") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (comicBold) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (comicBold) "CompanyApp - Comic Bold prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (comicBold) {
                ComicBoldProtoApp()
