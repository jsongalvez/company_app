...
import com.companyb.companyapp.proto.monthclose.MonthCloseProtoApp
...
fun main() {
    ...
    application {
        val protoMonthClose = System.getenv("COMPANYAPP_PROTO_MONTH_CLOSE") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoMonthClose) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoMonthClose) "CompanyApp — Month Close Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoMonthClose) {
                MonthCloseProtoApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
