...
import com.companyb.companyapp.proto.whiteboard.WhiteboardApp
...
fun main() {
    ...
    application {
        // #844 — whiteboard prototype gate; existing behavior preserved otherwise.
        val whiteboard = System.getenv("COMPANYAPP_PROTO_WHITEBOARD") == "true"
        val windowState =
            rememberWindowState(
                size = if (whiteboard) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (whiteboard) "CompanyApp — whiteboard prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (whiteboard) {
                WhiteboardApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
