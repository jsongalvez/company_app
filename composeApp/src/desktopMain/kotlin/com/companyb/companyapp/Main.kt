...
import com.companyb.companyapp.proto.carddeck.CardDeckProtoApp
...
fun main() {
    ...
    val cardDeck = System.getenv("COMPANYAPP_PROTO_CARD_DECK") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (cardDeck) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (cardDeck) "CompanyApp - Card Deck prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (cardDeck) {
                CardDeckProtoApp()
