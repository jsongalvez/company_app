...
import com.companyb.companyapp.proto.threecolumn.ThreeColumnApp
...
fun main() {
    ...
    application {
        val threeColumn = System.getenv("COMPANYAPP_PROTO_THREE_COLUMN") == "true"
        val windowState =
            rememberWindowState(
                size = if (threeColumn) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (threeColumn) "CompanyApp — three-column prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (threeColumn) {
                ThreeColumnApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
