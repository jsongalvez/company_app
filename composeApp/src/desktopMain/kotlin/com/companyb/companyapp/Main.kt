...
import com.companyb.companyapp.proto.franchisemap.FranchiseMapProtoApp
...
fun main() {
    ...
    val franchiseMap = System.getenv("COMPANYAPP_PROTO_FRANCHISE_MAP") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (franchiseMap) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (franchiseMap) "CompanyApp - Franchise Map prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (franchiseMap) {
                FranchiseMapProtoApp()
            } else {
                App()
            }
        }
