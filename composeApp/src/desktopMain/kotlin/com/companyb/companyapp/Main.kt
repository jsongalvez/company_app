...
import com.companyb.companyapp.proto.newsroomdesk.NewsroomDeskApp
...
fun main() {
    ...
    application {
        val newsroomDesk = System.getenv("COMPANYAPP_PROTO_NEWSROOM_DESK") == "true"
        val windowState =
            rememberWindowState(
                size = if (newsroomDesk) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (newsroomDesk) "CompanyApp — newsroom-desk prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (newsroomDesk) {
                NewsroomDeskApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
