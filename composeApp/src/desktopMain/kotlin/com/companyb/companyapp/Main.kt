...
import com.companyb.companyapp.proto.limboroom.LimboRoomApp
...
fun main() {
    ...
    application {
        val limboRoom = System.getenv("COMPANYAPP_PROTO_LIMBO_ROOM") == "true"
        val windowState =
            rememberWindowState(
                size = if (limboRoom) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (limboRoom) "CompanyApp — limbo-room prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (limboRoom) {
                LimboRoomApp()
