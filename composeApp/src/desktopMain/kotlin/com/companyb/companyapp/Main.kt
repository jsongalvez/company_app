...
import com.companyb.companyapp.proto.departuresboard.DeparturesBoardApp
...
fun main() {
    ...
    application {
        val departuresBoard = System.getenv("COMPANYAPP_PROTO_DEPARTURES_BOARD") == "true"
        val windowState =
            rememberWindowState(
                size = if (departuresBoard) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (departuresBoard) "CompanyApp — departures-board prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (departuresBoard) {
                DeparturesBoardApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
