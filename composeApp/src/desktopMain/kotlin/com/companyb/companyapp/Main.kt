...
import com.companyb.companyapp.proto.decogold.DecoGoldProtoApp
...
fun main() {
    ...
    val decoGold = System.getenv("COMPANYAPP_PROTO_DECO_GOLD") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (decoGold) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (decoGold) "CompanyApp - Deco-Gold prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (decoGold) {
                DecoGoldProtoApp()
