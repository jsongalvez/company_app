...
import com.companyb.companyapp.proto.a11ymax.A11yMaxProtoApp
...
fun main() {
    ...
    application {
        val protoA11yMax = System.getenv("COMPANYAPP_PROTO_A11Y_MAX") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoA11yMax) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoA11yMax) "CompanyApp — A11y Max Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoA11yMax) {
                A11yMaxProtoApp(onExit = ::exitApplication)
            } else {
                App()
            }
        }
