...
import com.companyb.companyapp.proto.sessionscribe.SessionScribeProtoApp
...
fun main() {
    ...
    val sessionScribe = System.getenv("COMPANYAPP_PROTO_SESSION_SCRIBE") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (sessionScribe) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (sessionScribe) "CompanyApp — session-scribe prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (sessionScribe) {
                SessionScribeProtoApp()
            } else {
                App()
            }
        }
