...
import com.companyb.companyapp.proto.beaconstatus.ProtoBeaconStatusApp
...
fun main() {
    ...
    application {
        val protoBeaconStatus = System.getenv("COMPANYAPP_PROTO_BEACON_STATUS") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoBeaconStatus) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoBeaconStatus) "CompanyApp — beacon-status prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoBeaconStatus) {
                ProtoBeaconStatusApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
