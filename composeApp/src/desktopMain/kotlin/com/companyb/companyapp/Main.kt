...
import com.companyb.companyapp.proto.clinicallight.ClinicalLightApp
...
fun main() {
    ...
    application {
        val clinicalLight = System.getenv("COMPANYAPP_PROTO_CLINICAL_LIGHT") == "true"
        val windowState =
            rememberWindowState(
                size = if (clinicalLight) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (clinicalLight) "CompanyApp — clinical-light prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (clinicalLight) {
                ClinicalLightApp()
