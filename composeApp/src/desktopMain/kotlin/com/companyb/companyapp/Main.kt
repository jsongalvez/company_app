...
import com.companyb.companyapp.proto.questlog.QuestLogApp
...
fun main() {
    ...
    val questLog = System.getenv("COMPANYAPP_PROTO_QUEST_LOG") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (questLog) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (questLog) "CompanyApp // quest-log" else "CompanyApp",
            state = windowState,
        ) {
            if (questLog) {
                QuestLogApp()
