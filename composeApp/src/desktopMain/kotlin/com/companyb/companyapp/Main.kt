...
import com.companyb.companyapp.proto.calendarops.CalendarOpsProtoApp
...
fun main() {
    ...
    application {
        val protoCalendarOps = System.getenv("COMPANYAPP_PROTO_CALENDAR_OPS") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoCalendarOps) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoCalendarOps) "CompanyApp — Calendar Ops Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoCalendarOps) {
                CalendarOpsProtoApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
