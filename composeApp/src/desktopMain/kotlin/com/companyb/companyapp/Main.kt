...
import com.companyb.companyapp.proto.chalkboard.ChalkboardProtoApp
...
fun main() {
    ...
    application {
        val protoChalkboard = System.getenv("COMPANYAPP_PROTO_CHALKBOARD") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoChalkboard) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoChalkboard) "CompanyApp — Chalkboard Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoChalkboard) {
                ChalkboardProtoApp()
