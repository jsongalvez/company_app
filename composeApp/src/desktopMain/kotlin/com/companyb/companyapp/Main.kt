...
import com.companyb.companyapp.proto.readonlyaudit.ReadonlyAuditProtoApp
...
fun main() {
    ...
    val readonlyAudit = System.getenv("COMPANYAPP_PROTO_READONLY_AUDIT") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (readonlyAudit) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (readonlyAudit) "CompanyApp - Readonly Audit prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (readonlyAudit) {
                ReadonlyAuditProtoApp()
            } else {
                App()
            }
        }
