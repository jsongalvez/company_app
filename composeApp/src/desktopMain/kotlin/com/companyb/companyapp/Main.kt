...
import com.companyb.companyapp.proto.warroom.WarRoomApp
...
fun main() {
    ...
    application {
        val warRoom = System.getenv("COMPANYAPP_PROTO_WAR_ROOM") == "true"
        val windowState =
            rememberWindowState(
                size = if (warRoom) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (warRoom) "CompanyApp — war-room prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (warRoom) {
                WarRoomApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
