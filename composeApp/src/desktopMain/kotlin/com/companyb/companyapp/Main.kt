...
import com.companyb.companyapp.proto.checkinkiosk.CheckinKioskApp
...
fun main() {
    ...
    application {
        val checkinKiosk = System.getenv("COMPANYAPP_PROTO_CHECKIN_KIOSK") == "true"
        val windowState =
            rememberWindowState(
                size = if (checkinKiosk) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (checkinKiosk) "CompanyApp — checkin-kiosk prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (checkinKiosk) {
                CheckinKioskApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
