...
import com.companyb.companyapp.proto.ganttbranch.GanttBranchApp
...
fun main() {
    ...
    application {
        val ganttBranch = System.getenv("COMPANYAPP_PROTO_GANTT_BRANCH") == "true"
        val windowState =
            rememberWindowState(
                size = if (ganttBranch) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (ganttBranch) "CompanyApp — gantt-branch prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (ganttBranch) {
                GanttBranchApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
