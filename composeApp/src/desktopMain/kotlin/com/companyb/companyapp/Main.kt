...
import com.companyb.companyapp.proto.weekreview.ProtoWeekReviewApp
...
fun main() {
    ...
    application {
        val protoWeekReview = System.getenv("COMPANYAPP_PROTO_WEEK_REVIEW") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoWeekReview) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoWeekReview) "CompanyApp — Week Review Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoWeekReview) {
                ProtoWeekReviewApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
