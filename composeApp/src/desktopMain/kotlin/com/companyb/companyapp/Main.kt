...
import com.companyb.companyapp.proto.daybreakroutine.DaybreakApp
...
fun main() {
    ...
    val daybreakRoutine = System.getenv("COMPANYAPP_PROTO_DAYBREAK_ROUTINE") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (daybreakRoutine) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (daybreakRoutine) "CompanyApp // daybreak-routine" else "CompanyApp",
            state = windowState,
        ) {
            if (daybreakRoutine) {
                DaybreakApp()
