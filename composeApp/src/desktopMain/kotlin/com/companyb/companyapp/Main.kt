...
import com.companyb.companyapp.proto.ledgerbook.LedgerBookProtoApp
...
fun main() {
    ...
    application {
        val protoLedgerBook = System.getenv("COMPANYAPP_PROTO_LEDGER_BOOK") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoLedgerBook) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoLedgerBook) "CompanyApp — Ledger Book Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoLedgerBook) {
                LedgerBookProtoApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
