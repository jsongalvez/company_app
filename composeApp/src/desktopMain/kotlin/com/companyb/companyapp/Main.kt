...
import com.companyb.companyapp.proto.projectormode.ProtoProjectorModeApp
...
fun main() {
    ...
    application {
        val protoProjectorMode = System.getenv("COMPANYAPP_PROTO_PROJECTOR_MODE") == "true"
        val windowState =
            rememberWindowState(
                size = if (protoProjectorMode) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (protoProjectorMode) "CompanyApp — Projector Mode Prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (protoProjectorMode) {
                ProtoProjectorModeApp(onBack = ::exitApplication)
            } else {
                App()
            }
        }
