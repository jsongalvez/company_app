...
import com.companyb.companyapp.proto.arcadecabinet.ArcadeCabinetProtoApp
...
fun main() {
    ...
    application {
        val arcadeCabinet = System.getenv("COMPANYAPP_PROTO_ARCADE_CABINET") == "true"
        val windowState =
            rememberWindowState(
                size = if (arcadeCabinet) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (arcadeCabinet) "CompanyApp - Arcade Cabinet prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (arcadeCabinet) {
                ArcadeCabinetProtoApp()
            } else {
                App()
            }
        }
