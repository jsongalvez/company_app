...
import com.companyb.companyapp.proto.pixelretro.PixelRetroProtoApp
...
fun main() {
    ...
    val pixelRetro = System.getenv("COMPANYAPP_PROTO_PIXEL_RETRO") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (pixelRetro) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (pixelRetro) "CompanyApp - Pixel-Retro prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (pixelRetro) {
                PixelRetroProtoApp()
