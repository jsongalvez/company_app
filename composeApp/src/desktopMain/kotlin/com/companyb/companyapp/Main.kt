...
import com.companyb.companyapp.proto.stickerbook.StickerBookProtoApp
...
fun main() {
    ...
    val stickerBook = System.getenv("COMPANYAPP_PROTO_STICKER_BOOK") == "true"
    application {
        val windowState =
            rememberWindowState(
                size = if (stickerBook) DpSize(1280.dp, 800.dp) else DpSize(1024.dp, 768.dp),
                position = WindowPosition(Alignment.Center),
            )

        Window(
            onCloseRequest = ::exitApplication,
            title = if (stickerBook) "CompanyApp - Sticker Book prototype" else "CompanyApp",
            state = windowState,
        ) {
            if (stickerBook) {
                StickerBookProtoApp()
